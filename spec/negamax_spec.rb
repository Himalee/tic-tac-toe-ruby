...
describe Negamax do
  ...
  context "'X' move: returns cell given grid with" do
    it "one move left" do
      ...
      expect(@negamax.get_best_move(grid, "X")).to eql(7)
    end
   ...
   it "five moves left" do
      ...
      expect(@negamax.get_best_move(grid, "X")).to eql(4)
    end

    it "five moves left - trap move" do
      grid = ["O", 1, 2, 3, "X", "O", 6, 7, "X"]
      expect(@negamax.get_best_move(grid, "X")).to eql(2).or(eq(6))
    end
  end
  ...
  context "'O' move: returns cell given grid with" do
    it "five moves left - stops opponenet from winning" do
      ...
      expect(@negamax.get_best_move(grid, "O")).to eql(1)
    end
  end
end
